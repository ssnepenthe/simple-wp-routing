...
function twr_should_enable_cache()
{
    $enable = $_REQUEST['twr_enable_cache'] ?? false;

    return filter_var($enable, FILTER_VALIDATE_BOOL);
}
...
(function () {
    // HTTP method tests.
    if (twr_should_enable_cache()) {
        $cache = new RewriteCollectionCache(__DIR__ . '/var/cache', 'http-method-cache.php');
        $rewrites = $cache->get();
    } else {
...
(function () {
    // Orchestrator tests.
    if (twr_should_enable_cache()) {
        $cache = new RewriteCollectionCache(__DIR__ . '/var/cache', 'orchestrator-cache.php');
        $rewrites = $cache->get();
    } else {
...
(function () {
    // Responder tests.
    if (twr_should_enable_cache()) {
        $cache = new RewriteCollectionCache(__DIR__ . '/var/cache', 'responder-cache.php');
        $rewrites = $cache->get();
    } else {
